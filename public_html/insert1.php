...
for   ($row = 1; $row < mysql_num_fields($result);$row++ ) {
	$fields =  mysql_fetch_field($result, $row)->name  . ', ' .  $fields;
	$newentry = '"' . array_reduce($_POST[mysql_fetch_field($result, $row)->name], "myfunction") . '", ' . $newentry;
}
$newentry = substr($newentry, 0, strlen($newentry ) - 2);
$fields = substr($fields, 0, strlen($fields  )-2);
if ($_POST['input']=='Add new') {
	$sql="INSERT INTO samples ($fields) VALUES ($newentry)";
...
for   ($row = 1; $row < mysql_num_fields($result);$row++ ) {
	if (   array_reduce($_POST[mysql_fetch_field($result, $row)->name], "myfunction")!='')
	{
	$link1 = mysql_fetch_field($result, $row)->name ;
	$link2 = array_reduce($_POST[mysql_fetch_field($result, $row)->name], "myfunction");
	$fields =  $fields .  mysql_fetch_field($result, $row)->name  . ' LIKE "%' . array_reduce($_POST[mysql_fetch_field($result, $row)->name], "myfunction") . '%" AND ';
}
}
...
$sql = "Select * FROM samples Where " . $fields . ";";
...
echo "<h1>Edit Database Entries</h1><br>";
echo "<table border='1'>";
...
echo "<tr><td><input type = 'submit' value='Create New Entry'></td></tr></form>";
echo "</table>";
echo "<br>";
